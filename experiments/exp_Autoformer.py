...
for kk in keys:
    ...
    models = [Autoformer(h=horizon,input_size=12,scaler_type='standard',decoder_layers=2,hidden_size=88, max_steps=500,batch_size=2,learning_rate=5e-4,loss=MSE() )]
    ...
    #Future
    models = [Autoformer(h=23,input_size=12, scaler_type='standard',decoder_layers=2,hidden_size=88,max_steps=500,batch_size=2,learning_rate=5e-4, loss=MSE())]
    nf = NeuralForecast(models=models, freq='MS')
    nf.fit(df=df)
