...
for kk in keys:
    ...
    if kk=="mf":
        models = [LSTM(h=horizon,scaler_type='standard',max_steps=500,encoder_hidden_size=88,decoder_hidden_size=32, decoder_layers=4,batch_size=2, context_size=5)]
    else:
        models = [LSTM(h=horizon, scaler_type='standard', max_steps=500, encoder_hidden_size=88, decoder_hidden_size=32,
                       decoder_layers=4, batch_size=2, context_size=5, loss=MSE())]
    ...
    mae_score = mean_absolute_error(Y_test_df['y'], Y_hat_df['LSTM'])
    rmse_score = mean_squared_error(Y_test_df['y'], Y_hat_df['LSTM'], squared=False)
    ...
    #Future
    if kk=="mf":
        models = [LSTM(h=23,scaler_type='standard',max_steps=500,encoder_hidden_size=88,decoder_hidden_size=32,decoder_layers=4,batch_size=2, context_size=5)]
    else:
        models = [LSTM(h=23, scaler_type='standard', max_steps=500, encoder_hidden_size=88, decoder_hidden_size=32,
                       decoder_layers=4, batch_size=2, context_size=5,loss=MSE())]
    nf = NeuralForecast(models=models, freq='MS')
    nf.fit(df=df)
    ...
    plt.plot(Y_hat_df.index, Y_hat_df['LSTM'], label=f'{method} Pred', color='orange')
    plt.plot(Y_future_df.index, Y_future_df['LSTM'], label=f'{method} Future', color='maroon')
