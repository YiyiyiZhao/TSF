...
for kk in keys:
    ...
    if kk=="mf":
        models = [DLinear(h=horizon,input_size=12,moving_avg_window=7, scaler_type='standard',max_steps=600,batch_size=2, loss=MSE())]
    else:
        models = [DLinear(h=horizon, input_size=12, moving_avg_window=7, scaler_type='standard', max_steps=600, batch_size=2)]
    ...
    #Future
    if kk=="mf":
        models = [DLinear(h=23,input_size=12, moving_avg_window=7, scaler_type='standard',max_steps=600,batch_size=2, loss=MSE())]
    else:
        models = [DLinear(h=23, input_size=12, moving_avg_window=7, scaler_type='standard', max_steps=600, batch_size=2)]
    nf = NeuralForecast(models=models, freq='MS')
    nf.fit(df=df)
