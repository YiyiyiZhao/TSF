...
for kk in keys:
    ...
    models = [NLinear(h=horizon,input_size=4, scaler_type='standard',max_steps=700,batch_size=4,learning_rate=5e-4, loss=MSE())]
    ...
    #Future
    models = [NLinear(h=23,input_size=4,  scaler_type='standard',max_steps=700,batch_size=4,learning_rate=5e-4, loss=MSE())]
    nf = NeuralForecast(models=models, freq='MS')
    nf.fit(df=df)
